  ...
  return (
    <div>
      <section className="mx-auto max-w-5xl px-5 pt-32 pb-16 sm:px-8 sm:pt-40">
        ...
        <Reveal delay={0.15}>
          <p className="font-display mt-6 max-w-xl text-2xl font-medium tracking-tight text-balance">
          ...
          <div className="mt-10 flex flex-col gap-10">
            {strategyAi.applied.cases.map((c, i) => (
              <Reveal key={c.title} delay={Math.min(i * 0.03, 0.15)}>
                <div className="grid gap-3 border-t border-paper/15 pt-8 sm:grid-cols-[1fr_2fr] sm:gap-8">
                  <div>
                    <h3 className="font-display text-xl font-semibold tracking-tight text-balance">
                      {c.title}
                    </h3>
                    <p className="mt-2 text-sm text-paper/50">{c.tool}</p>
                  </div>
                  <p className="text-paper/80 text-pretty leading-relaxed">{c.body}</p>
                </div>
              </Reveal>
            ))}
          </div>
